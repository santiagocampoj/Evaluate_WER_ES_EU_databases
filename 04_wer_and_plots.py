...
def save_data_and_summarize(data, high_wer_sentences, directory):
    ...
    histogram_data, bin_edges = np.histogram(df['WER'], bins=55)
    histogram_df = pd.DataFrame({
        'Bin Start': bin_edges[:-1],
        'Bin End': bin_edges[1:],
        'Frequency': histogram_data
    })
    histogram_csv_path = os.path.join(plots_directory, f'{plot_name}_wer_histogram_data_{MODEL_STRING}.csv')
